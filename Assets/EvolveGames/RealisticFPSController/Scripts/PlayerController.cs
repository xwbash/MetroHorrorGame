    ...
    public class PlayerController : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (Cursor.lockState == CursorLockMode.Locked && canMove)
            {
                ...
                if (isRunning && Moving) cam.m_Lens.FieldOfView = Mathf.Lerp(cam.m_Lens.FieldOfView, RunningFOV, SpeedToFOV * Time.deltaTime);
                else cam.m_Lens.FieldOfView = Mathf.Lerp(cam.m_Lens.FieldOfView, InstallFOV, SpeedToFOV * Time.deltaTime);
            }
            ...
            else if (!Physics.Raycast(GetComponentInChildren<CinemachineVirtualCamera>().transform.position, transform.TransformDirection(Vector3.up), out CroughCheck, 0.8f, 1))
            {
            ...
            if(WallDistance != Physics.Raycast(GetComponentInChildren<CinemachineVirtualCamera>().transform.position, transform.TransformDirection(Vector3.forward), out ObjectCheck, HideDistance, LayerMaskInt) && CanHideDistanceWall)
            {
                WallDistance = Physics.Raycast(GetComponentInChildren<CinemachineVirtualCamera>().transform.position, transform.TransformDirection(Vector3.forward), out ObjectCheck, HideDistance, LayerMaskInt);
